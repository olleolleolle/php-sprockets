...
If you pass @false@ to the render method, @render()@ will not echo the javascript and return it as string instead.

You can also ignore the second constructor parameter and use setters instead. Setters always return the
object itself so you can chain the commands, if you like. 

Example: @$sprocket->setDebugMode(true)->setAutoRender(false)->render()@
